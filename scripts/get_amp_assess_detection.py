"""
    get_amp_assess_detection.py
    Written by ZBH, 7/21/2025

    Performs cuts on f150 and f090 timestreams, combines results
    into one timestream, and calculates best fit amplitude for all
    frequencies for data and many sims - can be used to decide if
    we have detected an oscillation or not.

    Currently marginalizes over phase and saves only best fit amplitude.
    Could easily be modified to save best fit amplitude and phase by switching
    sim_process() to use timestream_analysis.calc_best_fit_amp_and_phase()
    instead of timestream_analysis.calc_marg_amp().
...
parser.add_argument("output_dir_tag", help=
    ...
    E.g. assess_detection_results_<output_dir_tag> will be output directory",type=str)
...
output_dir_path = output_dir_root + "assess_detection_results_" + output_tag + '/'
...
def sim_process(input_fs, As, phases, angles, errorbars, durations, mean_times,
                output_dir_path, output_tag, sim_num):
    """Generates a simulated timestream from the real data, then calculates
       best fit amplitudes for each split. Saves output array of best fit amp at each freq
       to a npy file for further processing."""
    ...
    sim_amps = ta.calc_marg_amp(input_fs, As, phases, 
                                sim_timestream,errorbars,
                                durations,mean_times)

    # Save outputs
    output_fname1 = output_dir_path + output_tag + f'_sim{sim_num:05}_amplitudes.npy'
    np.save(output_fname1, sim_amps)
...
if rank==0:
    ...
    config_output_name = output_dir_path + 'assess_detection_config_' + output_tag + ".yaml"
    ...
    logger.info("Running real timestream on process 0")
    try:
        data_amps = ta.calc_marg_amp(input_fs, As, phases, 
                                        sorted_combined_angles,sorted_combined_errorbars,
                                        sorted_combined_obs_duration,sorted_combined_mean_time)
        results_output_fname1 = output_dir_path + output_tag + '_real_amplitudes.npy'
        np.save(results_output_fname1, data_amps)
    except Exception as e:
        logger.error(f"Real timestream failed with error {e}")
else:
    ...
    for i in range(rank-1, n_sims, size-1):
        ...
        try:
            sim_process(input_fs, As, phases, 
                        sorted_combined_angles, sorted_combined_errorbars, 
                        sorted_combined_obs_duration, sorted_combined_mean_time,
...
logger.info(f"Finished running get_amp_assess_detection.py. Output is in: {output_dir_path}")
stop_time = time.time()
